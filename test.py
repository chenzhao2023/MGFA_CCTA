...
from models import unet3d, MGFA ,AGFA, LCT_DR_UNet_new, csnet_3d, casnet_3d, vnet, DenseVoxelNet
...
from test_cfg import get_test_config
...
if __name__ == '__main__':
    ...
    MODEL_NAME = "unet"  # vnet csnet densenet MGFA casnet LCT AGFA

    cfg = get_test_config(MODEL_NAME)

    overlap = cfg.overlap  # [16, 20, 20]
    patch_size = cfg.patch_size  # [128, 160, 160]
    batch_size = cfg.batch_size  # 1
    num_workers = cfg.num_workers  # 1
    class_thrd = cfg.class_thrd  # 0.5
    back_thrd = cfg.back_thrd  # 3000
    ...
    if MODEL_NAME == "MGFA":
        ...
        net = unet3d.UNet3D().to(device)
        checkpoint_path = r''
    elif MODEL_NAME == "vnet":
        net = vnet.VNet().to(device)
        checkpoint_path = r''
    elif MODEL_NAME == "csnet":
        net = csnet_3d.CSNet3D().to(device)
        checkpoint_path = r''
    elif MODEL_NAME == "casnet":
        net = casnet_3d.CASNet3D().to(device)
        checkpoint_path = r''
    elif MODEL_NAME == "LCT":
        net =LCT_DR_UNet_new.LCT_new_DR_UNet_new().to(device)
        checkpoint_path = r''
    elif MODEL_NAME == "AGFA":
        net =AGFA.AGFANet().to(device)
        checkpoint_path = r''
    elif MODEL_NAME == "densenet":
        net =DenseVoxelNet.DenseVoxelNet().to(device)
        checkpoint_path = r''
    else:
    ...
    for i, (inpute, labele, cous, name, shapes,spacing) in tqdm(enumerate(test_loader)):
        ...
        pred_back = backpreprcess_min(pred_merged_volume,thrd=back_thrd)
        ...
        print(f"Precision: {precision:.4f},{all_precision / num:.4f}")
        print(f"Recall: {recall:.4f},{all_recall / num:.4f}")
        print(f"hausdorff: {hsd:.4f},{all_hsd / num:.4f}")
