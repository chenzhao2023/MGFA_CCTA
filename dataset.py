...
def extract_number_img(filename):
    match = re.search(r'(\d+)_(\d+)_image', filename)
    if match:
        group = match.group(1)
        number = int(match.group(2))
        return group, number
...
class VesselsSegmentionDataSetsave_t(Dataset):
    def __init__(self, rootdir, rate):
        ...
        self.contoursdir = os.path.join(rootdir, r'contours_noerode')
...
class VesselsSegmentionDataSet512(Dataset):
    ...
    def __getitem__(self, item):
        if torch.is_tensor(item):
            item = item.tolist()
        # np.random.seed(int(self.patients[item][4:]))
        ...
        label = nib.load(os.path.join(self.data_dir, str(self.patients[item]) , 'label.nii.gz'))
        labels_header = label.header
        label = label.get_fdata()
        # label = cv2.resize(label, (256, 256))
        label[np.where(label > 0)] = 1
        cou[np.where(cou > 0)] = 1
        # cou = np.zeros_like(cou)
        ...
        cou = cou.unsqueeze(0)
        name = self.patients[item]
        pixdim = labels_header.get("pixdim")  # NIfTI 的 pixdim 包含体素间距
        spacing = (pixdim[3], pixdim[1], pixdim[2]) # 对应 x, y, z 间距
        spacing = np.array(
