...
if __name__ == "__main__":
    # agff = AGFF(128,256,128)
    # x = torch.randn(1, 128, 8, 8, 8)  # Example input
    # y = torch.randn(1, 256, 4, 4, 4)  # Example input
    # out = agff(x,y)
    # print(out.shape)
    model = UNet3D(1,1).cuda()
    total_params = sum(p.numel() for p in model.parameters())
    print(f"Total number of parameters: {total_params}")
    x = torch.randn(1, 1, 128, 160, 160).cuda()
    out = model(x)
    print(out.shape)
